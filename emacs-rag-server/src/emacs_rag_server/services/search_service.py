...
from ..models.database import query_by_text, query_by_vector, query_hybrid
...
def text_search(query: str, *, limit: int = 5) -> List[Dict[str, Any]]:
    ...
    """
    # Query database using FTS5
    raw_results = query_by_text(query, n_results=limit)

    # Format results
    results = _format_text_results(raw_results)

    return results


def _format_text_results(payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Transform FTS5 database results to API format.

    Args:
        payload: Raw database results from FTS5 query
    ...
    results = []
    for doc, metadata, distance in zip(documents, metadatas, distances):
        # For FTS5, distance is actually BM25 score (higher is better)
        # We use it directly as the score
        result = {
            'content': doc,
            'source_path': metadata['source_path'],
            'chunk_index': metadata['chunk_index'],
...
def hybrid_search(query: str, *, limit: int = 5, vector_weight: float = 0.5, rerank: bool = True) -> List[Dict[str, Any]]:
    """
    ...
    # Query database with hybrid search
    raw_results = query_hybrid(
        query,
        query_embedding,
